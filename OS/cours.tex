...
\begin{lem}[]
    If $ \phi  $ is diff, convex, increasing, then $ sign(h^{\star, \phi }) = f^{\star, Bayes} $ with $ h^{\star, \phi }  \in  argmin_h R^\phi (h)$ 
    ...
    \begin{proof}[Preuve : ]
        \begin{enumerate}
            ...
            \item Define $ H_\phi (p, \eta ) := \eta \phi (-p) + (1 - \eta ) \phi (p) $ and $ p^{\star , \phi }(\eta ) = argmin H_\phi (p, \eta ) $ (assuming existence for now) \\
...
\end{align*}
where \begin{itemize}
    \item $ h^{\star , \phi }_\mathcal{F} \in argmin R^\phi (h) $ 
...
\begin{note}[]
    ...
    \begin{enumerate}
        \item Method 1 : \begin{align*}
            \hat{\theta } &= \frac{1}{n} \sum_{i=1}^{n} Z_i = argmin_\theta \frac{1}{2n} \sum_{i=1}^{n} (Z_i - \theta )^2  \\
            \theta ^\star &= argmin \frac{1}{2}\mathbb{E}[ (\theta - Z) ^2] = \mathbb{E}[Z]
        \end{align*}
...
\begin{itemize}
    ...
    [0, 1] $ 
    \item If we add diff (tangent lie below) we have $ F(\theta ^\prime ) \geq F(\theta ) + < \nabla F(\theta ) , \theta ^\prime - \theta  > \forall \theta , \theta ^\prime $ 
    \item (increasing slopes) $ < \nabla F(\theta ) - \nabla F(\theta ^\prime ), \theta - \theta ^\prime > \geq 0 $ ($ \nabla F $ is said to be a monotone operator )
...
\begin{itemize}
    ...
    \item (\textbf{"well"}increasing slopes) $ < \nabla F(\theta ) - \nabla F(\theta ^\prime ), \theta - \theta ^\prime > \geq 0 + \mu \left\| \theta - \theta ^\prime  \right\|  $
...
\begin{lem}[Descent lemma]
    Assume that $ F $  is L-Smoot. Therefore $ \forall \theta , \theta ^\prime \in  $ dommain of f 
    \[
        F(\theta ^\prime ) \leq  F(\theta ) + < \nabla F(\theta ) , \theta ^\prime  - \theta > + \frac{L}{2} \left\| \theta ^\prime  - \theta  \right\| 
    .\]
    \begin{proof}[Preuve : ]
        \begin{align*}
            F (\theta ^\prime ) 
                ...
                &\leq F(\theta ) \mathbf{+} < \nabla F(\theta ), \theta ^\prime  - \theta > + \int_{0}^{1} \left\| \nabla F(\theta + t(\theta ^\prime  - \theta )) - \nabla F(\theta ) \right\| \left\| \theta  ^\prime - \theta  \right\| dt \\
                &\leq F(\theta ) \mathbf{+} < \nabla F(\theta ), \theta ^\prime  - \theta > +  \int_{0}^{1} t L \left\| \theta ^\prime  - \theta  \right\|^2 dt \\
                &\leq F(\theta ) \mathbf{+} < \nabla F(\theta ), \theta ^\prime  - \theta > + \frac{1}{2} L \left\| \theta  ^\prime  - \theta  \right\| ^2 _2
        \end{align*}
    \end{proof}
\end{lem}
...
\begin{enumerate}
    \item \begin{align*}
        F(\theta ) &\leq F(\theta ^\star ) + < \nabla F(\theta ^\star ) , \theta  - \theta ^\star > + \frac{L}{2} \left\| \theta - \theta ^\star  \right\| ^2 \\
        F(\theta ) - F(\theta ^\star ) &\leq \frac{L}{2} \left\| \theta  - \theta ^\star  \right\| ^2 
    \end{align*}
    \item 
    \[
        \min_\theta  F(\theta ) \leq \min _\theta F(\theta ) + < \nabla F(\theta ), \theta ^\prime - \theta >  + \frac{L}{2}\left\| \theta ^\prime - \theta  \right\| ^2
    .\]
    $ \min _\theta F(\theta ) + < \nabla F(\theta ), \theta ^\prime - \theta >  + \frac{L}{2}\left\| \theta ^\prime - \theta  \right\| ^2 $  is reach for $ \theta ^\prime  = \theta - \frac{1}{L} \nabla F(\theta ) $ 
    \begin{align*}
        &\leq F(\theta ) + < \nabla F(\theta ), \theta  - \frac{1}{L} \nabla F(\theta ) - \theta  > + \frac{L}{2} \left\| \theta  - \frac{1}{L} \nabla F(\theta ) - \theta  \right\| ^2 \\
...
\begin{note}[]
    ...
    \[
        \forall \theta ^\prime, F(\theta ^\prime ) \geq F(\theta ) + < \eta , \theta ^\prime - \theta > 
    .\]
...
$ \theta ^\star = argmin F $ assuming existence and uniqueness. 
...
\begin{itemize}
    ...
    \item Line search : optimal step size at each iteration. $ \gamma _{t} = argmin _{\gamma > 0} F(\theta _{t-1} - \gamma \nabla F(\theta _{t-1}))) $. You can forget about that case in online algo!
\end{itemize}
...
Indeed $ \frac{\partial (F \circ \theta )}{\partial t}(t) = < \nabla F(\theta (t)), \dot{\theta }(t) > = - \left\| \nabla F(\theta (t)) \right\| ^2  $ 

\begin{thm}[]
    For $ F $ an L-Smooth. for $ \gamma _t = \gamma , \forall t $ with $ \gamma < 2/L $ 
    \[
        F(\theta _t) - F(\theta ^\star ) \leq \frac{\left\| \theta _0 tjeta
        ^\star  \right\| }{2 \gamma (1 - \frac{\gamma L}{2}) T }
    .\]
    For $ \gamma = \frac{1}{L} $ we have 
    \[
        F(\theta _t) - F(\theta ^\star ) \leq \frac{\left\| \theta _0 tjeta
        ^\star  \right\| }{2 \gamma (1 - \frac{\gamma L}{2}) T } = \frac{L \left\| \theta _0 - \theta ^\star  \right\| ^2}{T}
    .\]
    ...
    \begin{note}[Interpolation of GD with $ \gamma = \frac{1}{L} $ ]
        Note that \begin{align*}
            \tilde{\theta }_t 
                &= argmin F(\tilde{\theta }_{t-1}) + < \nabla F(\tilde{\theta } _{t-1}), \theta  - \tilde{\theta }_{t-1} > + \frac{L}{2} \left\| \theta  - \tilde{\theta }_{t - 1} \right\| ^2 \\
    ...
    \begin{proof}[Preuve : ]
        \begin{align*}
            \left\| \theta _{t+1} - \theta ^\star  \right\| ^2 _2  
                ...
                &= \left\| \theta _t - \theta ^\star  \right\| ^2 _2 - 2 \gamma  < \nabla F(\theta _t), \theta _t - \theta ^\star > + \gamma ^2 \left\| \nabla F(\theta _t) \right\| ^2 _2
        \end{align*}
        Function convexe + L-Smooth : $ \left\| \nabla F(\theta ) \right\| ^2 \leq L < \nabla F(\theta ), \theta  - \theta ^\star > $. This is a consequence of the co-coercivity of $ \nabla F $ (with param $ 1/L $ )
        \begin{note}[Co-coercivity]
            $ F $ convex, L-Smooth, then $ \theta , \theta ^\prime  $ 
            \[
                < \nabla F(\theta ) - \nabla F(\theta ^\prime ), \theta  - \theta ^\prime \geq_{\text{co-coercivity}} \frac{1}{L} \left\| \nabla F(\theta ) - \nabla F(\theta ^\prime ) \right\| ^2 _2
            .\]
            \begin{proof}[Preuve : ]
                Define two function \begin{align*}
                    G(\theta ^\prime ) &= F(\theta ^\prime ) - < \nabla F(\theta ) , \theta ^\prime >  \\
                    H(\theta ^\prime ) &= F(\theta ) - < \nabla F(\theta ^\prime ), \theta > 
                \end{align*}
                $ G $ and $ H $ are smooth. $ \theta ^\prime = \theta  $ minimize $ \theta ^\prime  \mapsto G(\theta ^\prime ) $ and 
                \begin{align*}
                    F(\theta ^\prime) - F(\theta ) - < \nabla F(\theta ) , \theta ^\prime - \theta  >  
                        &= G(\theta ^\prime) - G(\theta ) \\
                ...
                \begin{align*}
                    F(\theta ) - F(\theta ^\prime ) - < \nabla F(\theta ^\prime ), \theta  - \theta ^\prime > &= H (\theta ) - H(\theta ^\prime ) \\
                    &\geq \frac{1}{2L} \left\| \nabla H(\theta )  \right\| ^2 \\
                    &= \frac{1}{2L} \left\| \nabla F(\theta ^\prime ) - \nabla F(\theta ) \right\| ^2
                \end{align*}
        ...
        \begin{align*}
            \left\| \theta _{t+1} - \theta ^\star  \right\| ^2 
                &= \left\| \theta _t - \theta ^\star  \right\| ^2 - 2 \gamma  < \nabla F(\theta _t), \theta _t - \theta ^\star > + \gamma  ^2 \left\| \nabla F(\theta _t) \right\| ^2 \\
                &\geq  \left\| \theta _t - \theta ^\star  \right\| ^2 - 2 \gamma ( 1 - \frac{\gamma L }{2}) < \nabla F(\theta -t) , \theta _t - \theta ^\star >  \\
                &\Rightarrow 2 \gamma (1 - \frac{\gamma L}{2}) < \nabla F(\theta _t) , \theta _t - \theta ^\star > \leq  \left\| \theta _{t+1} - \theta ^\star  \right\| ^2 - \left\| \theta _t - \theta ^\star  \right\| ^2  \\
