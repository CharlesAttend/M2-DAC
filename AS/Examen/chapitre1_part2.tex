\section{Limite probabilistique des gradients}
...
\begin{proposition}\label{prop5}
    Soit les réseaux \textit{res-1}, \textit{res-2} et \textit{res-3} définit dans la \Cref{tab:resnet_architectures}. Supposons l'\Cref{H1} satisfaite et $ \sigma  $ dérivable presque partout avec $ a \leq \sigma ^\prime \leq b $. Alors
    \begin{itemize}
        \item [(i)] L'\Cref{H4} est valide pour \textit{res-1}
        \item [(ii)] L'\Cref{H4} est valide pour \textit{res-2} et \textit{res-3} dès lors que les entrées de $ \sqrt{d} W_{k+1}, 0 \leq k \leq L-1 $ sont des variables aléatoires de variance unitaire, i.i.d, symétriques et indépendante de $ d $ et $ L $.
    \end{itemize}
\end{proposition}
