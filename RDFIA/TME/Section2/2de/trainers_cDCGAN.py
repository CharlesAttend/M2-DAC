...
def setup_trainer(
    ...
    def train_function(engine: Union[Engine, DeterministicEngine], batch: Any):
        ...
        y = torch.nn.functional.one_hot(labels).float().to(device)
        ...
        avg_real_score = yhat_real.mean().item()  # discriminator's output on real images
        avg_fake_score = yhat_fake.mean().item()  # discriminator's output on fake images
        avg_fake_score_generator = yhat_fake_generator.mean().item()  # discriminator's output on fake images

        metrics = {
            "epoch": engine.state.epoch,
            "errD": d_loss.item(),
            "errG": g_loss.item(),
            "D_x": avg_real_score,  # moyenne des logits
