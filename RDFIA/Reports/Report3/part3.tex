...
For further inversigation to observe the effect of ODIN technics, we tried to combine it with MC Dropout (by taking the mean of $ 100 $ stochastic forward passes), predictive entropy and mutual information. The results are presented in \Cref{fig:OOD_aupr_combo}. We can see that the combination of ODIN with MCD, predictive entropy and mutual information actualy decrease performance of Entropy and mutual information, with a score $ 0.3\% $ lower. Adding MCD to ODIN doesn't seem to have any effect. 
\begin{figure}[H]
    \centering
