...
\noindent Let's prove it analytically. With $\alpha = 0$ and $\beta = 1$, the computation of $\Sigma^{-1}$ simplifies as follows:
\begin{align*}
    \mathbf{\Sigma}^{-1} 
...
\[
    \mathbf{\Sigma} = \frac{1}{\det \mathbf{\Sigma}^{-1}} \begin{pmatrix}
        \sum x_i^2 & - \sum x_i \\
        - \sum x_i & N
    \end{pmatrix}
\]
Returning to our expression for $ \sigma^2_{pred} = \Phi (x^\star ) ^T \mathbf{\Sigma} \Phi (x^\star )$, we have:
\begin{align*}
    \sigma^2_{pred} = \Phi (x^\star ) ^T \mathbf{\Sigma} \Phi (x^\star ) &= \frac{1}{\det \mathbf{\Sigma}^{-1}} \begin{pmatrix}
        1 \\
        x^\star 
    ...
    \end{pmatrix}\begin{pmatrix}
        1 & x^\star 
    \end{pmatrix} \\
    &= \frac{1}{\det \mathbf{\Sigma}^{-1}} \begin{pmatrix}1 \\ x^\star \end{pmatrix} \begin{pmatrix}\sum_i x_i^2 - x^\star \sum_i x_i & x^\star N - \sum_i x_i\end{pmatrix} \\
    &= \frac{1}{\det \mathbf{\Sigma}^{-1}}\left(\sum_i x_i^2 - x^\star \sum_i x_i + {x^\star }^2 N - x^\star \sum_i x_i\right) \\
    &= \frac{1}{\det \mathbf{\Sigma}^{-1}}\left(\sum_i x_i^2 - 2 x^\star \sum_i x_i + {x^\star }^2 N\right) \\
    &= \frac{1}{\det \mathbf{\Sigma}^{-1}}\sum_i \left(x_i - x^\star \right)^2
\end{align*}
...
Even though we miss data points at the barycentre of our training data, the predictive variance is still minimized. 
