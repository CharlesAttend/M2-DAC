...
\Cref{fig:phi_gaussian} and \Cref{fig:phi_gaussian_hole} illustrate a comparison between the predictions made by a RBF Gaussian Regression model and the actual ground truth. Contrary to the last two models, we conclude different conclusions. Notably, the predictive variance isn't highest far from the training barycenter but at points where there is information. It fluctuates, with peaks and troughs corresponding to the areas where the density of training points varies. Thus, the model is closely aligned to the mean of the training data and for instance, we can see how it can impacts the sinusoidal learned curves, with a small shift .

% ptit shift dans la moyenne a un endroit et ça modifie la tête de la sinusoide, donc très fitted sur les données
% variance et prediction constante quand on est éloigné des points de train
