...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Comparaison of the bee eater class visualization using VGG16 (a) with a lot of regularization, (b) with regularization and (c) without regularization and starting from a base image of the class.} % (blurring and weight regularization)
    \label{fig:class_viz_reg}
\end{figure}

% préciser les valeurs de régu ?
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Same image after (a) 200 iterations, (b) 500 iterations, (c) 1000 iterations and (d) at 1000 iterations but without having performed regularization.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{.33\textwidth}
        \centering
        \includegraphics[width=.7\linewidth]{SqueezeNet/bird_animated_reg++_last_frame.png}
    ...
    \begin{subfigure}{.33\textwidth}
        \centering
        \includegraphics[width=.7\linewidth]{SqueezeNet/bird_animated_same_init_img_reg++_last_frame.png}
    ...
    \end{subfigure}
    \caption{Class visualization: (a) Last iteration starting from noise, (b) Last iteration starting from the same class, and (c) \href{figs/2b/bird_animated_same_init_img_reg++.gif}{Animation} starting from the same class to maximize the score for the bee eater class, all with strong regularization.}
...
We did the all same experiment using VGG16 this time. Visualization are much better because of the overall better performance of VGG16 as we saw in . % Maybe ref la figure saliency map avec VGG16 où il se trompe pas
TODO

% Parfois plus délicat de faire des bonnes visualizations, mais le résultat est clairement plus clair, on voit que le réseau est plus performant que l'autre 